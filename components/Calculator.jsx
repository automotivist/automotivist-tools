...
// Embeddable calculator — accepts preload props for programmatic pages
...
function buildShareCard(canvas, score, g, d) {
  ...
  const stats = [
    ...
    { l: 'TOTAL INTEREST', v: fmtK(d.interest) },
  ];
  const cw = (W - 160) / 3;
  stats.forEach((s, i) => {
...
// ── Main component ────────────────────────────────────────────
export default function Calculator({ preloadPayment, preloadSalary, slug, compact = false }) {
  ...
  function submitEmail() {
    ...
    fetch(`https://api.beehiiv.com/v2/publications/${BEEHIIV_PUB_ID}/subscriptions`, {
      ...
      body: JSON.stringify({
        ...
        utm_source: 'programmatic-seo',
        utm_medium: 'calculator',
        utm_campaign: slug || 'calculator-landing',
      }),
    }).catch(() => {});

    setSubmitted(true);

    // Build share card after fonts load
    document.fonts.ready.then(() => {
      if (canvasRef.current && results) {
        buildShareCard(canvasRef.current, results.score, results.gradeObj, results);
      }
    });
  }
  ...
  function shareX() {
    if (!results) return;
    const { score, gradeObj, ip, sp10 } = results;
        ...
        <div style={S.grid2}>
          <Field label="Monthly payment" error={errors.payment} prefix="$">
            <input style={{...S.input, ...(errors.payment ? S.inputErr : {})}}
              type="number" value={payment} onChange={e => setPayment(e.target.value)}
          ...
          <Field label={<span>Loan term — <span style={{ color: 'var(--amber-fill, #E8A020)', fontWeight: 700 }}>{term} months</span></span>}>
          ...
          <Field label="Monthly insurance" error={errors.insurance} prefix="$">
            <input style={{...S.input, ...(errors.insurance ? S.inputErr : {})}}
              type="number" value={insurance} onChange={e => setInsurance(e.target.value)}
              placeholder="175" min="0" />
          </Field>
        </div>
        <div style={{ marginTop: 16 }}>
          <Field label="Annual income (gross, before taxes)" error={errors.income} prefix="$">
      ...
      {gated && results && teaser && (
        <div ref={resultsRef} style={S.teaserCard}>
          ...
          <div style={S.statGrid}>
            ...
            <StatCell label="Monthly out-of-pocket" value={fmt(results.moTot)} sub="payment + insurance" color="amber" />
      ...
      {submitted && results && (
        <>
          ...
          <div style={S.card}>
            ...
            <BarRow label="Monthly payment + insurance" amount={fmt(results.moTot)} pct={Math.min(results.ip, 100)} note={fmtP(results.ip) + ' of gross monthly income'} color="#CC3232" />
            <BarRow label="Estimated true monthly cost (AAA all-in)" amount={fmt(results.trueMo)} pct={Math.min((results.trueMo / results.moInc) * 100, 100)} note="depreciation + maintenance + fuel + fees + insurance + payment" color="#E8A020" />
          ...
          <div style={S.scoreSection}>
            ...
            <div style={S.scoreStats}>
              ...
              <ScoreStat label="Total interest" value={fmtK(results.interest)} />
            </div>
...
function Field({ label, children, error, prefix, suffix }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <label style={{ fontSize: 12, fontWeight: 500, color: 'var(--light-muted, #6A6560)', fontFamily: 'var(--font-body)' }}>
        {label}
      </label>
      ...
      {error && <span style={{ fontSize: 11, color: 'var(--red-dark, #B03428)' }}>Required</span>}
    </div>
  );
}
...
function StatCell({ label, value, sub, color }) {
  ...
  return (
    <div style={{ padding: '20px 24px', borderRight: '1px solid var(--light-border, #D8D3C8)', borderBottom: '1px solid var(--light-border, #D8D3C8)' }}>
      <div style={{ fontSize: 11, color: 'var(--light-muted, #6A6560)', fontWeight: 500, marginBottom: 7 }}>{label}</div>
      <div style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(20px,4vw,28px)', fontWeight: 700, color: colorMap[color] || colorMap.amber }}>
        {value}
      </div>
      <div style={{ fontFamily: 'var(--font-display)', fontSize: 11, color: 'var(--light-dim, #A8A49C)', marginTop: 5 }}>{sub}</div>
    </div>
  );
}
  ...
  return (
    <div style={{ marginBottom: 18 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 7 }}>
        <span style={{ fontSize: 12, color: 'var(--light-muted, #6A6560)', fontWeight: 500 }}>{label}</span>
        <span style={{ fontFamily: 'var(--font-display)', fontSize: 17, fontWeight: 700, color }}>{amount}</span>
      </div>
      <div style={{ height: 6, background: 'var(--light-border2, #E5E0D8)', borderRadius: 3, overflow: 'hidden' }}>
        <div style={{ height: '100%', borderRadius: 3, background: color, width: width + '%', transition: 'width .9s cubic-bezier(.16,1,.3,1)' }} />
      </div>
      <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, color: 'var(--light-dim, #A8A49C)', marginTop: 5 }}>{note}</div>
    </div>
  );
}
...
function AssetRow({ name, rate, value, top, badge }) {
  return (
    <div style={{ background: top ? 'rgba(28,104,64,.03)' : 'var(--light-surface, #FFFFFF)', border: `1px solid ${top ? '#1C6840' : 'var(--light-border, #D8D3C8)'}`, borderRadius: 11, padding: '18px 20px', marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 1px 6px rgba(0,0,0,.04)' }}>
      <div>
        <div style={{ fontFamily: 'var(--font-display)', fontSize: 16, fontWeight: 700, color: 'var(--light-text, #17140D)' }}>{name}</div>
        <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, color: 'var(--light-dim, #A8A49C)', marginTop: 3 }}>{rate}</div>
      </div>
...
// ── Inline style system (light mode + brand dark for score) ──
const styles = {
  wrapper: { maxWidth: 680, margin: '0 auto' },
  card: {
    background: '#FFFFFF', border: '1px solid #D8D3C8',
    borderRadius: 14, padding: '28px 28px 24px', marginBottom: 18,
    boxShadow: '0 2px 12px rgba(0,0,0,.05)',
  },
  sectionLabel: {
    fontFamily: 'var(--font-display, "Barlow Condensed")', fontSize: 11, fontWeight: 600,
    letterSpacing: '.22em', textTransform: 'uppercase', color: '#A8A49C',
    marginBottom: 20, paddingBottom: 12, borderBottom: '1px solid #E5E0D8',
  },
  grid2: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 },
  input: {
    width: '100%', background: '#F5F2EC', border: '1.5px solid #D8D3C8',
    borderRadius: 8, padding: '11px 12px 11px 28px',
    fontFamily: 'var(--font-display, "Barlow Condensed")', fontSize: 17, fontWeight: 600,
    color: '#17140D', outline: 'none',
  },
  inputErr: { borderColor: '#B03428' },
  prefix: {
    position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)',
    fontFamily: 'var(--font-display)', fontSize: 15, fontWeight: 600, color: '#A8A49C', pointerEvents: 'none',
  },
  suffix: {
    position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)',
    fontFamily: 'var(--font-display)', fontSize: 13, color: '#A8A49C', pointerEvents: 'none',
  },
  ...
  calcBtn: {
    width: '100%', padding: 16, background: '#17140D', color: '#FAF8F4',
    border: 'none', borderRadius: 10,
    fontFamily: 'var(--font-display, "Barlow Condensed")', fontSize: 18, fontWeight: 700, letterSpacing: '.04em',
  ...
  gateCard: {
    background: '#FFFFFF', border: '1.5px solid rgba(232,160,32,.22)',
  ...
  emailInput: {
    ...
    background: '#F5F2EC', border: '1.5px solid #D8D3C8',
    borderRadius: 8, fontFamily: 'var(--font-body)', fontSize: 15, color: '#17140D', outline: 'none',
  },
  gateBtn: {
    padding: '12px 22px', background: '#E8A020', color: '#0C0A06',
    border: 'none', borderRadius: 8,
    fontFamily: 'var(--font-display)', fontSize: 16, fontWeight: 700, letterSpacing: '.04em',
  ...
  insightBox: {
    ...
    borderRadius: 11, padding: '18px 20px', margin: '18px 0',
  },
  scoreSection: {
    background: '#080808', borderRadius: 20, padding: '48px 32px 40px',
    textAlign: 'center', position: 'relative', overflow: 'hidden', marginBottom: 24,
  },
